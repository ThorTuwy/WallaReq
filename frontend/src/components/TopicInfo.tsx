...
async function saveChanges(topics:Topics,topicNameSignal:Accessor<string>,setTopicName:Setter<string>){
  ...
  await fetch('http://localhost:8000/topics/update', requestOptions)
    .then(response => response.json())
    .then(data => console.log(data));
  
  if(topics["name"]!=topicRealName){
    const response = await fetch(`http://127.0.0.1:8000/topics/remove?name=${topicRealName}`, {
      method: 'PUT',
      headers: {
...
export default function TopicInfo(props: any) {
  ...
  const deleteTopic = async () => {
    ...
    const response = await fetch(`http://127.0.0.1:8000/topics/remove?name=${topicNameSignal()}`, {
      method: 'PUT',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
